...
export const SignUp: React.FC<SignUpProps> = ({ initialError }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      // Simulate existing user email validation error matching Figma node #24496:39950
      if (email.toLowerCase().includes('johndoe') || email.toLowerCase().includes('already') || email.toLowerCase().includes('registered') || email === 'johndoe@gmail.com') {
      ...
      <main className="flex-1 flex items-center justify-center p-4 py-[40px] md:py-[73px]">
        <div className="w-full max-w-[520px] bg-[#FFFFFF] rounded-[24px] p-6 md:p-[40px] shadow-[0px_8px_24px_0px_rgba(29,22,29,0.05)] flex flex-col gap-[24px]">
          ...
          <div className="flex items-center justify-center gap-[4px] text-[14px]">
            <span className="font-[400] text-[#4B5675]">Already have an account?</span>
            <Link href="/login" className="font-[600] text-[#000000] hover:underline">
              Log in
            </Link>
          </div>
